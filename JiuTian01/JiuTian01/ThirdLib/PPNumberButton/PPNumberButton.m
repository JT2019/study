...
- (void)setupUI
{
    ...
    _textField.font = [UIFont systemFontOfSize:ScaleWidth6(24)];
...
{
    textField.text.length == 0 || textField.text.integerValue <= 0 ? _textField.text = @"0" : nil;
...
- (void)increase
{
    _textField.text.length == 0 ? _textField.text = @"0" : nil;
    NSInteger number = [_textField.text integerValue] + 1;
    
    if ([_textField.text isEqualToString:[NSString stringWithFormat:@"%d",_max]]) {

        [_increaseBtn setBackgroundImage:[UIImage imageNamed:@"++"] forState:UIControlStateNormal];
       
        _shakeAnimation ? [self shakeAnimation] : nil;
        NSLog(@"数量不能大于");
    }else{
        _textField.text = [NSString stringWithFormat:@"%ld", number];
         [_decreaseBtn setBackgroundImage:[UIImage imageNamed:@"-"] forState:UIControlStateNormal];
        _numberBlock ? _numberBlock(_textField.text) : nil;
        _delegate ? [_delegate PPNumberButton:self number:_textField.text] : nil;
    }
}
...
- (void)decrease
{
    ...
    {
        _textField.text = [NSString stringWithFormat:@"%ld", number];
        [_increaseBtn setBackgroundImage:[UIImage imageNamed:@"add+"] forState:UIControlStateNormal];
        _numberBlock ? _numberBlock(_textField.text) : nil;
        _delegate ? [_delegate PPNumberButton:self number:_textField.text] : nil;
    }
    else
